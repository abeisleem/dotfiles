- For medium-sized, multi-step tasks, proactively use the `subagent` tool for independent investigation or implementation that can run in parallel. Keep trivial or tightly coupled work in the main session, and retain final integration and verification there. For larger-sized, multi iteration and more effective as interactive tasks, use the opencode api to create new sessions for the user to work and steer in a dedicated session.
